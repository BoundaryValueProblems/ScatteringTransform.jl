...
Functors.@functor ScatteredFull (data, output)

function ScatteredFull(m::Int, k::Int, data, output)
    ScatteredFull{eltype(output),length(output)}(m, k, data, output)
end
...
Functors.@functor ScatteredOut (output,)
...
function ScatteredOut(m::Int, k::Int, output)
    ScatteredOut{eltype(output),length(output)}(m, k, output)
end
...
The resulting output of a scattering transform. `m` is the number of layers, while `k` is the actual signal dimension. 
`fixDim` is the size of any auxillary dimensions for the input (for example if the input is a list of `nEx` examples of 
length 100 each, then `fixDim=(nEx,)`, while `n` is a list of the subsampled signal sizes and `q` is a list of the paths, 
as generated by the scatteringTransform). Finally, the element type is `T`. Initializes all entries to zero.

      ScatteredOut(output, k = 1)
A less involved constructor given just a list or tuple of the output from each layer. `k` gives the signal dimension, as 
above, with the default that `k=1`.
  """
...
Given a `Scattered`, return the `pathLocs` where the `Scattered` is nonzero. `wholePath=true` if it returns the whole 
path, and not just the specific location in the signal. For example, if only `sc(pathLocs(1,(30,2)))` is nonzero, if 
`wholePath` is `true`, then `pathLocs(1,(2,))` will be returned while if `wholePath` is `false`, `pathLocs(1,(30,2))` 
will be returned instead. if `allTogetherInOne` is `false`, then each location is returned separately, otherwise they 
are joined into a single `pathLocs`.
"""
...
function makeSingle(prev, x::BitArray)
    ...
    if typeof(prev) <: Tuple{Vararg{Nothing}} 
        almostNull[findfirst(x)] = true
    end
    return (prev..., almostNull)
end